...
class ActionSequenceModel(nn.Module):
    def __init__(
        ...
        out_size = 2 * self.segment_len * self.action_dim
    ...
    def forward(self, obs: torch.Tensor) -> Tuple[torch.Tensor]:
        ...
        # net_out shape: (batch_size, 2 * segment_len * action_dim)
        half = net_out.shape[1] // 2
        mean_flat, logstd_flat = torch.split(net_out, [half, half], dim=1)

        # apply tanh to mean to keep in [-1, 1]
        mean_flat = torch.tanh(mean_flat)
        mean = mean_flat.view(batch_size, self.segment_len, self.action_dim)

        # clamp logstd then exponentiate to get std
        logstd_flat = torch.clamp(logstd_flat, LOGSTD_MIN, LOGSTD_MAX)
        std = torch.exp(logstd_flat).view(batch_size, self.segment_len, self.action_dim)        
    ...
    def distribution(self, obs: torch.Tensor) -> D.Distribution:
        ...
        mean, std = self.forward(obs)  # shapes: (B, segment_len, action_dim)
        # Create a Normal distribution over the last two dims (segment_len x action_dim)
        normal = D.Normal(mean, std)
        # Treat the last two dims as the event dims (multivariate over the whole sequence)
        dist = D.Independent(normal, reinterpreted_batch_ndims=2)
        return dist
        ### END CODE HERE ###
        #######################################################
    ...
    def act(self, obs: np.ndarray) -> np.ndarray:
        ...
        # Get the action sequence distribution (batch size 1)
        # Note: We wrap the single observation in a batch dimension for self.distribution
        with torch.no_grad():
            dist = self.distribution(np2torch(obs).unsqueeze(0))
            # Sample an action sequence: shape (1, segment_len, action_dim)
            action_sequence = dist.sample() 
            
        # Get the first action of the sequence: shape (action_dim,)
        first_action = action_sequence[0, 0] 
        
        # Convert to numpy and clip (already done in forward, but good practice here too)
        action_np = first_action.cpu().numpy()
        action_np = np.clip(action_np, -1.0, 1.0)
        return action_np


        #mean, std = self.forward(np2torch(obs).unsqueeze(0))  # shapes (1, segment_len, action_dim)
        #first_action = mean[0, 0]  # first action of the planned sequence
        #action_np = first_action.detach().cpu().numpy()
        #action_np = np.clip(action_np, -1.0, 1.0)
        #return action_np
...
class SFT(ActionSequenceModel):
    def update(self, obs: torch.Tensor, actions: torch.Tensor):
        ...
        # log_prob returns one scalar per batch element (log prob of whole sequence)
        logp = dist.log_prob(actions)  # shape: (batch,)
        loss = -logp.mean()  # maximize mean log-prob => minimize negative
...
class DPO(ActionSequenceModel):
    ...
    def update(
        ...
        dist = self.distribution(obs)
        logp_w = dist.log_prob(actions_w)
        logp_l = dist.log_prob(actions_l)

        # Reference policy log-probs (no grad)
        with torch.no_grad():
            ref_dist = ref_policy.distribution(obs)
            ref_logp_w = ref_dist.log_prob(actions_w)
            ref_logp_l = ref_dist.log_prob(actions_l)

        # score = beta * ( (logp_w - logp_l) - (ref_logp_w - ref_logp_l) )
        score = self.beta * ((logp_w - logp_l) - (ref_logp_w - ref_logp_l))

        # loss = - E[ log sigmoid(score) ] = - mean( logsigmoid(score) )
