...
class PolicyGradient(object):
    ...
    def init_policy(self):
        ...
        # 1) We need to build the raw feed forward MLP.
        # We are passing in observation dim vector. 
        # we re outputting logits or means for policy here.
        # Build an MLP that maps observations -> action logits/means
        ...
        # 2 & 3) Wrap the network with the appropriate policy distribution
        # if action space is discrete then use categorical policy
        # otherwise use gausian distribution policy
        if self.discrete:
            self.policy = CategoricalPolicy(self.network, self.device).to(self.device)
        else:
            self.policy = GaussianPolicy(self.network, self.action_dim, self.device).to(self.device)

        # 4) Optimizer for the policy parameters
        # Use adam optimizer with learning rate using parameter() method
    ...
    def get_returns(self, paths):
        ...
        for path in paths:
            rewards = path["reward"]
            ### START CODE HERE ###
            # Consider discoutn factor 
            gamma = self.config["hyper_params"]["gamma"]
            # Retrieve length of current trajectory 
            T = len(rewards)
            # Let's hold return values
            returns = np.zeros(T, dtype=np.float32)
            G = 0.0
            # compute discounted return for each timestep (backwards)
            # walk backwords now from time steps 
            for t in range(T - 1, -1, -1):
                G = rewards[t] + gamma * G
                # we need to store discounted returns 
                returns[t] = G
            ### END CODE HERE ###
    ...
    def normalize_advantage(self, advantages):
        ...
        # first we compute mean of all advantages
        mean = np.mean(advantages)
        # now we compute standard deviation 
        std = np.std(advantages) + 1e-8   # add epsilon to avoid division by zero
        # finally, we need to have each advantage normalized 
    ...
    def update_policy(self, observations, actions, advantages):
        ...
        # first we need to retrieve action distribution from the policy
        distribution = self.policy.action_distribution(observations)

        # Compute log probabilities of the taken actions
        # This will tell us likeliness of selected actions under current policy 
        log_probs = distribution.log_prob(actions)

        # Compute the policy gradient loss
        # we negate it since optimizer minimizes loss 
        loss = -(log_probs * advantages).mean()

        # Back progation using Standard PyTorch optimizer step
        self.optimizer.zero_grad() #remove old gradientds
        # now we compute new gradients
        loss.backward()
        # at last we need to update parameters 
        self.optimizer.step()
