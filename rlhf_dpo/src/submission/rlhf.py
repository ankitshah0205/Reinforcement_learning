...
class RewardModel(nn.Module):
    ...
    def forward(self, obs: torch.Tensor, action: torch.Tensor) -> torch.Tensor:
        ...
        rewards = self.r_min + (self.r_max - self.r_min) * rewards
    ...
    def compute_reward(self, obs: np.ndarray, action: np.ndarray) -> float:
        ...
        act_t = np2torch(action).unsqueeze(0)
        with torch.no_grad():
            reward = self.forward(obs_t, act_t)
        return reward.item()
        ### END CODE HERE ###
        #######################################################
    ...
    def update(self, batch: Tuple[torch.Tensor]):
        ...
        #########   5-10 lines.   ############
        ### START CODE HERE ###
        r1 = self.forward(obs1, act1).sum(dim=1)  # [batch_size]
        r2 = self.forward(obs2, act2).sum(dim=1)

        # probability of preferring trajectory 1
        #logits = torch.stack([r1, r2], dim=1)  # shape [batch_size, 2]


        # probability that traj1 is preferred over traj2
        logits = r2 - r1  # shape [batch_size]
        
        # apply cross-entropy loss
        #loss = F.cross_entropy(logits, label.long())
        
        # Bradley–Terry loss = BCE with logits
        loss = F.binary_cross_entropy_with_logits(logits, label.float(), reduction='sum')
        ### END CODE HERE ###
        #######################################################
