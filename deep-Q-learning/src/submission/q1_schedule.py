...
############################################################
# Problem 1a: learning rate scheduler
...
class LinearSchedule(object):
    ...
    def update(self, t):
        ...
        # Linearly decay epsilon from start_epsilon to final_epsilon
        # over decay_steps. After that, keep epsilon fixed at final_epsilon.
        # Ensure that steps are between zero and nsteps
        # we are linearly updating epsilon from begining to end. After nsteps epsilon stays at end
        bounded_step = min(max(t, 0), self.nsteps)
        # no decay. setting epsilon directly
        if self.nsteps == 0:
            # no decay. setting epsilon directly
            self.epsilon = self.eps_end
        else:
            # Assess where we are from 0 to 1 schedule
            progress = bounded_step / float(self.nsteps)
            # interpolation from beginning of epsilon to end of epsilon
            self.epsilon = self.eps_begin + (self.eps_end - self.eps_begin) * progress

        ### END CODE HERE ###


############################################################
# Problem 1b: e-greedy exploration
...
class LinearExploration(LinearSchedule):
    ...
    def get_action(self, best_action):
        ...
        # As per the formula below, probability of picking random action is e which is exploration
        # otherwise, we pick best action that is explitation
        # A ← argmax_a∈A Q(s,a)   with probability (1 - ϵ)
        # a ∈ A random            with probability ϵ
        
        # Generate a random number between 0 and 1. If this number is les than epsilon then we
        # will explore.
        if np.random.rand() < self.epsilon:
            # This ensures that agent tries new things openin gup possiblities for new learning. 
            return self.env.action_space.sample()
        # else pick the action that currently has the highest Q value. 
        # This refers to argmax part of the formula above.
        return best_action
