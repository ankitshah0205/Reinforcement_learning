...
class NatureQN(Linear):
    ...
    def initialize_models(self):
        ...
        ### START CODE HERE ###
        # Effective input channels is equal to raw channels multiplied by stacked frames
        in_channels = n_channels * self.config["hyper_params"]["state_history"]


        # Now, as per the hint, leverage nn functions.
        # Implement the function to build Q network and target network
        def build_network():
            return nn.Sequential(
                # First conv layer that reduced spatial sixe
                nn.Conv2d(in_channels, 32, kernel_size=8, stride=4, padding=2),
                nn.ReLU(),
                # Second conv layer that further compresses spatial size
                nn.Conv2d(32, 64, kernel_size=4, stride=2),
                nn.ReLU(),
                # Third conc layer that extract fine grained features with small kernel
                nn.Conv2d(64, 64, kernel_size=3, stride=1),
                nn.ReLU(),
                # Now flatten in to single vector
                nn.Flatten(),
                nn.Linear(64 * 7 * 7, 512),
                nn.ReLU(),
                # Final layer that maps one value per action 
                nn.Linear(512, num_actions)
            )

        # Q network that is used to select actions during model training
        self.q_network = build_network()
        # Target network that makes trainign stable
    ...
    def get_q_values(self, state, network):
        ...
        #Take the enviornment state batch (batch size, height, width, chanels*state_history
        # And, use permute to rearrang ethe dimensions so that shape becomes (batch size, chanels, height,width)
        state_permuted = state.permute(0, 3, 1, 2)
        # Decide which network to use dynamically based on the input parameter 
        selected_network = getattr(self, network)
        # forward pass - this will return each entry which is predicted q value for action
        out = selected_network(state_permuted)
