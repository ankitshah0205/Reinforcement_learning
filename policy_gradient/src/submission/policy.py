...
class BasePolicy(ABC):
    ...
    def act(self, observations):
        ...
        # first get the action distribution conditioned on current observations
        distribution = self.action_distribution(observations)

        # It is important in RL distribution to use stochasticity of policy
        # Sample actions from this distribution
        sampled_actions = distribution.sample()

        # Here numpy array is needed since env step() expects numpy array 
...
class CategoricalPolicy(BasePolicy, nn.Module):
    ...
    def action_distribution(self, observations):
        ...
        # Here logits represent the pre softmax probabilities`
        logits = self.network(observations)

        # Now we need to wrap logits in to categorical distribution.
...
class GaussianPolicy(BasePolicy, nn.Module):
    ...
    def __init__(self, network, action_dim, device):
        ...
        self.device = device
        ### START CODE HERE ###
        # Initialize log standard deviation as a learnable parameter
        # Figure out output dimension
        action_dim = network.output_dim if hasattr(network, "output_dim") else network[-1].out_features
        # Create log standard deviation
    ...
    def std(self):
        ...
        """
        ### START CODE HERE ###
        # Let's convert log standard deviation in to standard deviation 
    ...
    def action_distribution(self, observations):
        ...
        # First we need to pass observations mean vector from the network
        mean = self.network(observations)

        # Second we need to build Diagonal covariance matrix
        cov_matrix = torch.diag(self.std() ** 2)

        # Finally we need to construct multivariate normal distribution
        distribution = ptd.MultivariateNormal(loc=mean, covariance_matrix=cov_matrix)
